...
arrays such as [2,1,5] represent the serialization of a linked list with a head node value of 2, second node value of 1, 
...
public int[] nextLargerNodes(ListNode head) {
    ...
    while (head != null) {
        ans.add(head.val);
        while (!deque.isEmpty() && ans.get(deque.peekLast()) < head.val) {
            ans.set(deque.pollLast(), head.val);
        }
        deque.offerLast(i);
        i++;
        head = head.next;
