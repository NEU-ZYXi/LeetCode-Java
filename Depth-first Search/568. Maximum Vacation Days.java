...
Solution: use DFS and memo, for each city, we can either stay at current city or go to next connected city
O(nm),O(nm)
...
private int dfs(int[][] flights, int[][] days, int[][] memo, int curCity, int curWeek) {
    ...
    int ans = 0;
    ans = Math.max(ans, days[curCity][curWeek] + dfs(flights, days, memo, curCity, curWeek + 1));
    for (int i = 0; i < n; ++i) {
        if (flights[curCity][i] == 1) {
            ans = Math.max(ans, days[i][curWeek] + dfs(flights, days, memo, i, curWeek + 1));
        }
    }
    memo[curCity][curWeek] = ans;
    return ans;
}
