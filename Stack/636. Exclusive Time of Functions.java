...
public int[] exclusiveTime(int n, List<String> logs) {
    ...
    for (String log : logs) {
        String[] s = log.split("\\:");
        int id = Integer.valueOf(s[0]), timestamp = Integer.valueOf(s[2]);
        if (s[1].equals("start")) {
            deque.offerLast(new int[] {id, timestamp});
        } else {
            int[] prev = deque.pollLast();
            int duration = timestamp - prev[1] + 1;
            ans[id] += duration;
            if (!deque.isEmpty()) {
                ans[deque.peekLast()[0]] -= duration;
            }
        }
    }
    return ans;
