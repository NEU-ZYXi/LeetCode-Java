...
Solution: dp[i][j] means the minimum score of A[i...j]
          for each dp[i][j], for i<k<j, dp[i][j]=min(dp[i][k]+dp[k][j]+A[i]*A[j]*A[k]) which means choose each k to form a triangle
O(n^3),O(n^2)          
