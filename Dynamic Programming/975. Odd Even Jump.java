...
Solution: odd[i] means whether we could jump to end from ith position with odd numbered jump
          even[i] means whether we could jump to end from ith position with even numbered jump
          odd[i]=even[the index of smallest larger number]
          even[i]=odd[the index of largest smaller number]
          use treemap to track the index of numbers and quick get the larger or smaller number
O(nlogn),O(n)          
...
public int oddEvenJumps(int[] A) {
    ...
    map.put(A[n - 1], n - 1);
    for (int i = n - 2; i >= 0; --i) {
        Integer high = map.ceilingKey(A[i]), low = map.floorKey(A[i]);
        if (high != null) {
