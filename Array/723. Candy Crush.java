...
public int[][] candyCrush(int[][] board) {
    ...
    List<int[]> crush = new ArrayList<>();
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (board[i][j] != 0) {
                int type = board[i][j];
                if (j < m - 2 && board[i][j + 1] == type && board[i][j + 2] == type) {
                    for (int k = j; k < m && board[i][k] == type; ++k) {
                        crush.add(new int[] {i, k});
                    }
                }
                if (i < n - 2 && board[i + 1][j] == type && board[i + 2][j] == type) {
                    for (int k = i; k < n && board[k][j] == type; ++k) {
