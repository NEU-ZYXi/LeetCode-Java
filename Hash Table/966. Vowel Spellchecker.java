...
public String[] spellchecker(String[] wordlist, String[] queries) {
    ...
    for (String s : wordlist) {
        String lower = s.toLowerCase(), vowel = lower.replaceAll("[aeiou]", "#");
        lowers.putIfAbsent(lower, s);
        vowels.putIfAbsent(vowel, s);
    }
    for (int i = 0; i < queries.length; ++i) {
        ...
        String lower = s.toLowerCase(), vowel = lower.replaceAll("[aeiou]", "#");
        if (lowers.containsKey(lower)) {
            queries[i] = lowers.get(lower);
